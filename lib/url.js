...
// Default privacy frontend URLs
const DEFAULT_PRIVACY_URLS = {
  twitter: 'https://xcancel.com',
  reddit: 'https://redlib.com',
  tiktok: 'https://proxitok.pabloferreiro.es',
  twitch: 'https://safetwitch.uest.cc'
}

// Privacy frontend domain mappings
const PRIVACY_FRONTEND_DOMAINS = {
  'twitter.com': 'twitter',
  'x.com': 'twitter',
  'nitter.net': 'twitter',
  'nitter.it': 'twitter',
...
// URL path patterns for each service
const PRIVACY_URL_PATTERNS = {
  twitter: {
    // Twitter status URLs: /username/status/1234567890
    pattern: /^\/(\w+)\/status\/(\d+)/,
    transform: (match, user, id) => `/i/status/${id}`
  },
  reddit: {
    // Reddit post URLs: /r/subreddit/comments/abcde/title
    pattern: /^\/r\/([^\/]+)\/comments\/([^\/]+)\/?(.*)/,
    transform: (match, subreddit, id, rest) => `/r/${subreddit}/comments/${id}`
  },
  tiktok: {
    // TikTok URLs: /@username/video/1234567890123456789
    pattern: /^\/(@[^\/]+)\/video\/(\d+)/,
    transform: (match, user, id) => `/${user}/video/${id}`
  },
  twitch: {
    // Twitch URLs: /videos/1234567890 or /username
    pattern: /^(\/videos\/\d+|\/\w+)/,
    transform: (match, path) => path
  }
}

/**
 * Get privacy redirect URL based on user settings
 * @param {string} url - Original URL
 * @param {Object} settings - User privacy settings
 * @returns {string|null} - Redirected URL or null if no redirect needed
 */
export function getPrivacyRedirectUrl (url, settings = {}) {
  // Check if privacy redirects are enabled
  if (!settings.privacyRedirects) {
    return null
  }
  ...
  try {
    ...
    if (settings.privacyCustomRedirects && Array.isArray(settings.privacyCustomRedirects)) {
      for (const redirect of settings.privacyCustomRedirects) {
        if (redirect.source && redirect.target) {
          // Check if URL matches custom source pattern
          const sourceUrl = new URL(redirect.source)
          if (host === sourceUrl.host || host.endsWith('.' + sourceUrl.host)) {
            // Replace source domain with target domain
            let targetPath = pathname + search
            if (sourceUrl.pathname !== '/') {
              targetPath = targetPath.replace(sourceUrl.pathname, '')
            }
            return `${redirect.target}${targetPath}`
          }
        }
      }
    }

    // Check if this is a known privacy frontend service
    const service = PRIVACY_FRONTEND_DOMAINS[host]
    if (!service) {
      return null
    }

    // Get the privacy URL from settings or use default
    const privacyBaseUrl = settings[`privacyUrl${capitalize(service)}`] || DEFAULT_PRIVACY_URLS[service]
    if (!privacyBaseUrl) {
      return null
    }

    // Transform the URL path based on service
    const patternConfig = PRIVACY_URL_PATTERNS[service]
    if (!patternConfig) {
      return null
    }

    const match = pathname.match(patternConfig.pattern)
    if (!match) {
      return null
    }

    const newPath = patternConfig.transform(match, ...match.slice(1))
    return `${privacyBaseUrl}${newPath}${search}`
  } catch (err) {
    console.log('Error in getPrivacyRedirectUrl:', err)
    return null
  }
}

/**
 * Check if a URL should be handled by privacy redirect
 * @param {string} url - URL to check
 * @returns {boolean}
 */
export function isPrivacyRedirectable (url) {
  try {
    const urlObj = new URL(url)
    const host = urlObj.host.toLowerCase()
    return !!PRIVACY_FRONTEND_DOMAINS[host]
  } catch {
    return false
  }
}
