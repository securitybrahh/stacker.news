...
export const settingsSchema = object().shape({
  ...
  privacyUrlTwitter: string().nullable().url('must be a valid URL').max(200, 'must be at most 200 characters'),
  privacyUrlReddit: string().nullable().url('must be a valid URL').max(200, 'must be at most 200 characters'),
  privacyUrlTiktok: string().nullable().url('must be a valid URL').max(200, 'must be at most 200 characters'),
  privacyUrlTwitch: string().nullable().url('must be a valid URL').max(200, 'must be at most 200 characters'),
  privacyCustomRedirects: array().of(
    object({
      source: string().required('source domain is required').url('must be a valid URL'),
      target: string().required('target domain is required').url('must be a valid URL')
    })
  )
